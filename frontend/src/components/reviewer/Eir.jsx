...
const EIRApplications = () => {
  ...
  const fetchEIRs = async () => {
    ...
    try {
      const response = await axios.get(`https://start-x-server.vercel.app/review/${reviewer}/reviews`);
  ...
  const handleSubmitReview = async (appId) => {
    ...
    try {
      await axios.post(`https://start-x-server.vercel.app/review/eir/${appId}/${reviewer}`, {
        rating,
        comment
      });
