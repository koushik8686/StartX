...
export default function StartupDetails({ kyc, progress }) {
  ...
  async function fetchAd() {
    try {
      const response = await axios.get('https://start-x-server.vercel.app/ads/get/ad')
  ...
  async function handleAdClick(adId, link) {
    try {
      ...
      await axios.post(`https://start-x-server.vercel.app/ads/click/${adId}` , {startup})
          ...
          <div className="p-4 md:p-5">
            <div className="flex items-center space-x-3 pb-3">
              <div className="w-20 h-20 rounded-full overflow-hidden shadow-sm">
                <img 
                  src={`https://start-x-server.vercel.app/uploads/profiles/${kyc.profile_picture}`} 
